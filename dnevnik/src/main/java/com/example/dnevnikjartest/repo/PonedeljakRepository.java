...
public interface PonedeljakRepository extends JpaRepository<Ponedeljak, Integer> {
	...
	@Query(value="select * from ponedeljak p inner join odeljenje o on(p.id_odeljenje=o.id_odeljenje) inner join korisnik k on(k.id_korisnik=o.id_ucitelj) where k.id_korisnik = ?1", nativeQuery = true)
