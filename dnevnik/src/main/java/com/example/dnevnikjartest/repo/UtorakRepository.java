...
public interface UtorakRepository extends JpaRepository<Utorak, Integer> {
	...
	List<Utorak> findAll(int id);
		...
		@Query(value="select * from utorak p inner join odeljenje o on(p.id_odeljenje=o.id_odeljenje) inner join korisnik k on(k.id_korisnik=o.id_ucitelj) where k.id_korisnik = ?1", nativeQuery = true)
